...
import { useRevive5Categories } from "@/hooks/useRevive5";
import {
  useAllActiveRevive5Tasks,
  useRevive5TaskCompletions,
  useToggleRevive5TaskCompletion,
} from "@/hooks/useRevive5GoalTasks";

interface Revive5DailyTasksViewProps {
  teamId?: string;
}

const Revive5DailyTasksView = ({ teamId }: Revive5DailyTasksViewProps) => {
  const { user } = useAuth();
  ...
  const toggleCompletion = useToggleRevive5TaskCompletion();
  ...
  const goalsInCategory = activeCategory === "all" ? tasks : tasks.filter((t: any) => t.category_id === activeCategory);
        ...
        {tasksByCategory.map(({ category, tasks: catTasks }) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-[10px] opacity-75">{catCompleted}/{catTasks.length}</span>
            </button>
          );
        })}
      </div>
...
const CategoryTaskCard = ({ category, catTasks, isTaskCompleted, onToggle }: {
  ...
  return (
    <Card className={allDone ? "border-primary/30 bg-primary/5" : ""}>
      <CardHeader className="py-3 px-4 cursor-pointer select-none" onClick={() => setIsOpen(!isOpen)}>
        <CardTitle className="text-sm font-sans font-semibold flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs text-muted-foreground font-normal">{catCompleted}/{catTasks.length}</span>
            <ChevronDown className={cn("w-4 h-4 text-muted-foreground transition-transform duration-200", isOpen ? "rotate-0" : "-rotate-90")} />
          </div>
        </CardTitle>
      </CardHeader>
      <div className={cn("transition-all duration-200 overflow-hidden", isOpen ? "max-h-[2000px] opacity-100" : "max-h-0 opacity-0")}>
        <CardContent className="px-4 pb-3 pt-0 space-y-1">
          {catTasks.map((task: any) => {
            ...
            return (
              <div
                key={task.id}
                className={`flex items-center gap-3 p-2 rounded-md cursor-pointer transition-colors ${completed ? "opacity-60" : "hover:bg-secondary/50"}`}
                onClick={() => onToggle(task.id)}
              >
                <Checkbox checked={completed} onCheckedChange={() => onToggle(task.id)} className="pointer-events-none" />
                <div className="flex-1 min-w-0">
                  <p className={`text-sm font-sans ${completed ? "line-through text-muted-foreground" : ""}`}>{task.title}</p>
                  <p className="text-xs text-muted-foreground truncate font-sans">{task.goal?.title}</p>
                </div>
...
export default Revive5DailyTasksView;
